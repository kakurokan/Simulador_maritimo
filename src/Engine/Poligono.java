...
/**
 * A classe abstrata {@code Engine.Poligono} define uma estrutura básica para representar polígonos
 ...
 */
public class Poligono implements Obstaculo {
     ...
     *
     * @param pontos Um array de objetos {@code Engine.Ponto} que representam os vértices do polígono.
    ...
    public Poligono(Ponto[] pontos) {
        if (pontos.length < 3) {
            throw new IllegalArgumentException("Engine.Poligono:iv");
        }
     ...
     *
     * @param rota A rota (instância de {@code Engine.Route}) que contém os segmentos de reta
     *             a serem verificados para interseção com os lados do polígono.
     * @return Uma lista de pontos ({@code List<Engine.Ponto>}) que representam as interseções entre
